...
export default function UploadImage() {
  ...
  const handleFileChange = (e) => {
    ...
    if (selected) {
      setFile(selected);
      // Only generate an image preview for non-CSV uploads
      if (!isBank) {
  ...
  const handleSend = async () => {
    ...
    // Bank CSV goes to Render-CSV/; everything else goes to Render/
    const endpoint = isBank ? `${Backend_url}Render-CSV/` : `${Backend_url}Render/`;
    ...
    try {
      ...
      toast.success(
        isBank ? "CSV Uploaded Successfully ✅" : "Image Uploaded Successfully ✅"
      );
      ...
      console.error(error);
      toast.error(isBank ? "Failed To Upload CSV ❌" : "Failed To Upload Image ❌");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-6">
      ...
      <div className="bg-white shadow-xl rounded-2xl p-6 w-full max-w-md">
        <h1 className="text-2xl font-bold text-center mb-6">
          Upload or Capture Image of <br />({key_name} invoice)
        </h1>
        ...
        <button
          ...
          onClick={handleSend}
          className={`w-full mt-6 py-3 rounded-xl text-white font-semibold flex items-center justify-center ${
            file && !loading ? "bg-black hover:bg-gray-800" : "bg-gray-400 cursor-not-allowed"
          }`}
        >
          {loading ? (
            <div className="flex items-center gap-2">
